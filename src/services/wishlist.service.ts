...
export class WishlistService {
  ...
  async getWishlist(userId: string): Promise<GeneralResponse<IWishlist>> {
    try {
      ...
      throw new ResponseError(500, "Failed to retrieve wishlist");
    }
  }
  ...
  async addToWishlist(data: AddToWishlistData): Promise<GeneralResponse<IWishlist>> {
    try {
      ...
      if (exists) {
        throw ResponseError.conflict('Item already exists in wishlist');
      }
      ...
      if (itemType === WishlistItemType.PRODUCT) {
        ...
        if (!product.isActive) {
          throw ResponseError.badRequest("Produk tidak aktif dan tidak dapat ditambahkan ke wishlist.");
        }
        ...
        if (!auction) {
          throw ResponseError.notFound("Auction not found");
        }

        // Check if auction is still active
        const now = new Date();
        if (now < auction.startDate) {
          throw ResponseError.badRequest("Auction has not started yet");
        }
        if (now > auction.endDate) {
          throw ResponseError.badRequest("Lelang sudah berakhir dan tidak dapat ditambahkan ke wishlist.");
        }
        ...
        throw ResponseError.badRequest("Invalid item type");
      }
      ...
      throw new ResponseError(500, "Failed to add item to wishlist");
    }
  }
  ...
  async removeFromWishlist(userId: string, itemId: string, itemType: WishlistItemType): Promise<GeneralResponse<IWishlist | null>> {
    try {
      ...
      if (!wishlist) {
        throw ResponseError.notFound("Wishlist not found or item doesn't exist");
      }
      ...
      throw new ResponseError(500, "Failed to remove item from wishlist");
    }
  }
  ...
  async clearWishlist(userId: string): Promise<GeneralResponse<IWishlist | null>> {
    try {
      ...
      if (!wishlist) {
        throw ResponseError.notFound("Wishlist not found");
      }
      ...
      throw new ResponseError(500, "Failed to clear wishlist");
    }
  }
  ...
  async checkItemInWishlist(userId: string, itemId: string, itemType: WishlistItemType): Promise<GeneralResponse<{ exists: boolean }>> {
    try {
      ...
      throw new ResponseError(500, "Failed to check item in wishlist");
    }
  }
  ...
  > {
    try {
      ...
      throw new ResponseError(500, "Failed to get wishlist statistics");
    }
  }
  ...
  async getItemsByType(userId: string, itemType: WishlistItemType): Promise<GeneralResponse<IWishlistItem[]>> {
    try {
      ...
      throw new ResponseError(500, "Failed to get items by type");
    }
  }
    ...
    try {
      ...
      if (itemType === WishlistItemType.PRODUCT) {
        ...
        if (!product) {
          ...
          throw ResponseError.notFound("Product not found and removed from wishlist");
        }
        ...
        if (!auction) {
          ...
          await wishlistRepository.removeItem(userId, itemId, itemType);
          throw ResponseError.notFound("Auction not found and removed from wishlist");
        }
      ...
      throw new ResponseError(500, "Failed to sync wishlist item");
    }
  }
}
