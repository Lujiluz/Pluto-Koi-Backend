...
import { AuthenticatedRequest, AUTH_COOKIE_NAME, ADMIN_AUTH_COOKIE_NAME } from "../interfaces/auth.interface.js";

/**
 * Extract token from request (cookie or Authorization header)
 * @param req - The request object
 * @param isAdmin - If true, extract from admin cookie; if false, from user cookie
 */
const extractToken = (req: AuthenticatedRequest, isAdmin: boolean = false): string | null => {
  // Determine which cookie to use based on the context
...
/**
 * Legacy admin route authentication - checks role after authenticateToken
 * @deprecated Use authenticateAdminToken instead for proper cookie separation
 */
 ...
 * Uses admin-specific cookie (admin_auth_token) to allow concurrent user/admin sessions
 * Also validates session exists in database
 */
export const authenticateAdminToken = async (req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> => {
  try {
    const token = extractToken(req, true); // true = isAdmin
